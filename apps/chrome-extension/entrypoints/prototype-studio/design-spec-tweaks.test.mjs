...
test('edits complete numeric scales and raw palette only within the closed token boundary', async () => {
  const { applyDesignSpecTweak } = await moduleUnderTest()
  let draft = applyDesignSpecTweak(spec(), { key: 'fontSizeScale', value: [96, 12, 12, 14] })
  draft = applyDesignSpecTweak(draft, { key: 'fontWeightScale', value: [900, 400, 700] })
  draft = applyDesignSpecTweak(draft, { key: 'lineHeightScale', value: [32, 20] })
  draft = applyDesignSpecTweak(draft, { key: 'spacingScale', value: [32, 4, 8] })
  draft = applyDesignSpecTweak(draft, { key: 'radiusScale', value: [16, 4, 8] })
  draft = applyDesignSpecTweak(draft, { key: 'opacityScale', value: [1, .35, .35] })
  draft = applyDesignSpecTweak(draft, { key: 'rawColor', index: 0, value: '#112233' })
  assert.deepEqual(draft.typography.fontSizeScale, [12, 14, 96])
  assert.deepEqual(draft.typography.fontWeightScale, [400, 700, 900])
  assert.deepEqual(draft.typography.lineHeightScale, [20, 32])
  assert.deepEqual(draft.spacing.scale, [4, 8, 32])
  assert.deepEqual(draft.borders.radiusScale, [4, 8, 16])
  assert.deepEqual(draft.effects.opacities, [.35, 1])
  assert.equal(draft.colors[0].value, '#112233')
  assert.equal(applyDesignSpecTweak(draft, { key: 'rawColor', index: 99, value: '#000000' }), draft)
  assert.equal(applyDesignSpecTweak(draft, { key: 'rawColor', index: 0, value: 'url(https://unsafe.test)' }), draft)
  assert.equal(applyDesignSpecTweak(draft, { key: 'spacingScale', value: [] }), draft)
})

test('only allows effects already extracted by the trusted design spec and closed layout modes', async () => {
  const { applyDesignSpecTweak } = await moduleUnderTest()
  const original = { ...spec(), effects: { shadows: ['0 8px 24px rgba(15, 23, 42, .12)'], gradients: ['linear-gradient(135deg, #fff, #eef3ff)'], opacities: [.5] } }
  let draft = applyDesignSpecTweak(original, { key: 'surfaceShadow', value: original.effects.shadows[0] })
  draft = applyDesignSpecTweak(draft, { key: 'primaryGradient', value: original.effects.gradients[0] })
  draft = applyDesignSpecTweak(draft, { key: 'layoutPatterns', value: ['grid', 'sticky'] })
  assert.equal(draft.effects.semantic.surfaceShadow, original.effects.shadows[0])
  assert.equal(draft.effects.semantic.primaryControlGradient, original.effects.gradients[0])
  assert.deepEqual(draft.responsive.layoutPatterns, ['grid', 'sticky'])
  assert.equal(applyDesignSpecTweak(draft, { key: 'surfaceShadow', value: '0 0 999px red; background:url(x)' }), draft)
  assert.equal(applyDesignSpecTweak(draft, { key: 'layoutPatterns', value: ['anything'] }), draft)
  assert.equal(applyDesignSpecTweak(draft, { key: 'surfaceShadow', value: undefined }).effects.semantic.surfaceShadow, undefined)
})
