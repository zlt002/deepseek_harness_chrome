...
function directedQuestion(kind: KnowledgeKind, question: string, resumed: boolean): string { const instruction = resumed ? '这是同一远程检索会话的追问。请在已有上下文上继续回答，不要无必要地从头扫描仓库或知识库。' : kind === 'code' ? '请直接返回从所选远程代码仓库检索到的事实、文件路径和代码依据。' : '请直接返回从所选知识范围检索到的事实和引用依据。'; const language = /[\u3400-\u9fff]/u.test(question) ? '所有面向用户的流式内容和最终答案都必须使用简体中文；工具名、代码标识符和文件路径可保留原文。即使转述后的问题包含英文，也不要用英文叙述。' : 'Use the same language as the user question for all user-visible streaming content and the final answer.'; return `${instruction}${language}若用户要原文摘录，一次只返回一个文件或一个函数的核心片段；不要并行检索多个文件，也不要把多个大文件全文塞进同一次答案。最终答案只保留事实和引用，不要把思考过程写进最终答案。检索计划、当前正在查的仓库或知识、工具选择和进度可通过独立过程事件流式返回。用户问题：${question}` }
