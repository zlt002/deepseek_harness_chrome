export default defineContentScript({
  ...
  main() {
    ...
    let presentationRuntimeReady: Promise<void> | undefined
    const officeRuntimeBudgetMs = (action: unknown): number => action === 'probe'
      ? 400
      : ['inspect_write', 'write'].includes(String(action)) ? 20_000 : 8_000
    ...
    function loadPresentationRuntime(): Promise<void> {
      if (presentationRuntimeReady !== undefined) return presentationRuntimeReady
      presentationRuntimeReady = new Promise((resolve, reject) => {
        const script = document.createElement('script')
        script.src = chrome.runtime.getURL('/office-presentation-runtime.js')
        script.onload = () => { script.remove(); resolve() }
        script.onerror = () => { script.remove(); reject(new Error('unsupported: presentation runtime adapter could not load')) }
        ;(document.head ?? document.documentElement).append(script)
      })
      return presentationRuntimeReady
    }

    function invokePresentationRuntime(request: Record<string, unknown>, timeoutMs = 8_000): Promise<unknown> {
      return new Promise((resolve, reject) => {
        const id = crypto.randomUUID()
        const requestEvent = 'deepseek-harness-office-presentation-request/v1'
        const responseEvent = 'deepseek-harness-office-presentation-response/v1'
        const timeout = setTimeout(() => { window.removeEventListener(responseEvent, receive); reject({ code: 'timeout', message: 'Timed out waiting for the WebEdit presentation runtime.' }) }, timeoutMs)
        const receive = (event: Event): void => {
          const detail = (event as CustomEvent<unknown>).detail
          if (!detail || typeof detail !== 'object' || (detail as { id?: unknown }).id !== id) return
          clearTimeout(timeout); window.removeEventListener(responseEvent, receive)
          const payload = detail as { ok?: unknown; result?: unknown; error?: unknown }
          if (payload.ok === true) resolve(payload.result)
          else reject(payload.error ?? { code: 'runtime_error', message: 'WebEdit presentation operation failed' })
        }
        window.addEventListener(responseEvent, receive)
        window.dispatchEvent(new CustomEvent(requestEvent, { detail: { id, ...request } }))
      })
    ...
    chrome.runtime.onMessage.addListener((message: unknown, _sender, sendResponse) => {
      ...
      if ((message as { type?: unknown }).type === 'office-document/v1') {
        ...
        void loadDocumentRuntime().then(() => invokeDocumentRuntime({
          ...
          // writes 20s so a slow-but-succeeding patch is not reported as a
          // timeout while the document actually changed, while staying under
          // the native connector's 15s REQUEST_TIMEOUT_MS.
        }, officeRuntimeBudgetMs(input.action))).then((result) => sendResponse({ ok: true, result })).catch((error: unknown) => {
      ...
      if ((message as { type?: unknown }).type === 'office-spreadsheet/v1') {
        const input = message as { action?: unknown; range?: unknown; sheetName?: unknown; query?: unknown; matchCase?: unknown; matchEntireCell?: unknown; searchBy?: unknown; offset?: unknown; limit?: unknown; index?: unknown; fieldName?: unknown; axis?: unknown; cellType?: unknown; resource?: unknown; operation?: unknown; payload?: unknown; precondition?: unknown }
        if (!['context', 'active_sheet', 'selection', 'used_range', 'workbook_info', 'range', 'range_features', 'search', 'sheets', 'worksheet_protection', 'write_preflight', 'filter_state', 'filter_values', 'list_charts', 'chart', 'list_pivots', 'pivot', 'pivot_field_items', 'defined_names', 'capabilities', 'view', 'print_settings', 'outline', 'dimensions', 'special_cells', 'debug_runtime', 'probe_range_api', 'inspect_write', 'write', 'probe'].includes(String(input.action))) {
        ...
        void loadSpreadsheetRuntime().then(() => invokeSpreadsheetRuntime({
          ...
          ...(input.index === undefined ? {} : { index: input.index }), ...(input.fieldName === undefined ? {} : { fieldName: input.fieldName }), ...(input.axis === undefined ? {} : { axis: input.axis }), ...(input.cellType === undefined ? {} : { cellType: input.cellType }),
          ...(input.resource === undefined ? {} : { resource: input.resource }), ...(input.operation === undefined ? {} : { operation: input.operation }), ...(input.payload === undefined ? {} : { payload: input.payload }), ...(input.precondition === undefined ? {} : { precondition: input.precondition }),
        }, officeRuntimeBudgetMs(input.action))).then((result) => sendResponse({ ok: true, result })).catch((error: unknown) => {
      ...
      if ((message as { type?: unknown }).type === 'office-presentation/v1') {
        const input = message as { action?: unknown; slideIndex?: unknown; resource?: unknown; operation?: unknown; payload?: unknown; precondition?: unknown }
        if (!['get_context', 'selection', 'get_text_boxes', 'inspect_write', 'write', 'probe', 'inspect_capabilities'].includes(String(input.action))) {
          sendResponse({ ok: false, error: { code: 'invalid_range', message: 'a valid presentation action is required' } }); return false
        }
        void loadPresentationRuntime().then(() => invokePresentationRuntime({
          action: input.action, ...(input.slideIndex === undefined ? {} : { slideIndex: input.slideIndex }),
          ...(input.resource === undefined ? {} : { resource: input.resource }), ...(input.operation === undefined ? {} : { operation: input.operation }),
          ...(input.payload === undefined ? {} : { payload: input.payload }), ...(input.precondition === undefined ? {} : { precondition: input.precondition }),
        }, officeRuntimeBudgetMs(input.action))).then((result) => sendResponse({ ok: true, result })).catch((error: unknown) => {
          const typed = error && typeof error === 'object' && typeof (error as { code?: unknown }).code === 'string' ? error : { code: 'runtime_error', message: error instanceof Error ? error.message : 'WebEdit presentation operation failed' }
          sendResponse({ ok: false, error: typed })
        })
        return true
      }
