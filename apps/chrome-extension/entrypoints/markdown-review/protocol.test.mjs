...
test('review port accepts only exact versioned request and response shapes', () => {
  ...
  assert.equal(protocol.isMarkdownReviewPortResponse({ v: 1, type: 'markdown-review-snapshot-response', requestId: 'request-1', ok: true, snapshot: { ...snapshot, pmdPrd: true } }), true)
  assert.equal(protocol.isMarkdownReviewPortResponse({ v: 1, type: 'markdown-review-snapshot-response', requestId: 'request-1', ok: true, snapshot: { ...snapshot, pmdPrd: false } }), false)
  ...
  const actionRequest = { v: 1, type: 'markdown-review-session-action-request', requestId: 'request-action', reviewId: 'review-1', harnessSessionId: 'session-1', resourceId: 'resource-1', displayPath: 'docs/a.md', revision: 'revision-1', fingerprint: 'fingerprint-1', action: 'rewrite' }
  assert.equal(protocol.isMarkdownReviewPortRequest(actionRequest), true)
  assert.equal(protocol.isMarkdownReviewPortRequest({ ...actionRequest, unexpected: true }), false)
  const { fingerprint: _fingerprint, ...missingFingerprint } = actionRequest
  assert.equal(protocol.isMarkdownReviewPortRequest(missingFingerprint), false)
  assert.equal(protocol.isMarkdownReviewPortResponse({ v: 1, type: 'markdown-review-session-action-response', requestId: 'request-action', ok: true, action: 'rewrite', targetSessionId: 'session-1', targetSessionTitle: 'PRD 会话', status: 'draft_ready' }), true)
  assert.equal(protocol.isMarkdownReviewPortResponse({ v: 1, type: 'markdown-review-session-action-response', requestId: 'request-action', ok: true, action: 'rewrite', targetSessionId: 'session-1', targetSessionTitle: 'PRD 会话', status: 'queued' }), false)
  assert.equal(protocol.isMarkdownReviewPortResponse({ v: 1, type: 'markdown-review-session-action-response', requestId: 'request-action', ok: true, action: 'accept', targetSessionId: 'session-1', targetSessionTitle: 'PRD 会话', status: 'draft_ready' }), true)
  assert.equal(protocol.isMarkdownReviewPortResponse({ v: 1, type: 'markdown-review-session-action-response', requestId: 'request-action', ok: true, action: 'accept', targetSessionId: 'session-1', targetSessionTitle: 'PRD 会话', status: 'processing' }), false)
  const ratingRequest = { v: 1, type: 'markdown-review-rating-request', requestId: 'rating-request-1', reviewId: 'review-1', rating: 4 }
  assert.equal(protocol.isMarkdownReviewPortRequest(ratingRequest), true)
  assert.equal(protocol.isMarkdownReviewPortRequest({ ...ratingRequest, rating: 0.5 }), true)
  assert.equal(protocol.isMarkdownReviewPortRequest({ ...ratingRequest, rating: 4.5 }), true)
  assert.equal(protocol.isMarkdownReviewPortRequest({ ...ratingRequest, rating: 0.4 }), false)
  assert.equal(protocol.isMarkdownReviewPortRequest({ ...ratingRequest, rating: 4.4 }), false)
  assert.equal(protocol.isMarkdownReviewPortRequest({ ...ratingRequest, rating: 6 }), false)
  assert.equal(protocol.isMarkdownReviewPortResponse({ v: 1, type: 'markdown-review-rating-response', requestId: 'rating-request-1', ok: true, rating: 4 }), true)
  assert.equal(protocol.isMarkdownReviewPortResponse({ v: 1, type: 'markdown-review-rating-response', requestId: 'rating-request-1', ok: true, rating: 0 }), false)
  assert.equal(protocol.isMarkdownReviewPortResponse({ v: 1, type: 'markdown-review-deliver-response', requestId: 'request-2', ok: true, deliveryId: 'selection-1', targetSessionId: 'session-2', targetSessionTitle: '当前会话', status: 'queued' }), true)
