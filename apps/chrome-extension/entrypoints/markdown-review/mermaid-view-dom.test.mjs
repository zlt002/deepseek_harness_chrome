...
test('Mermaid viewer zooms, resets, and pans the canvas without touching the SVG source', () => {
  ...
  assert.equal(canvas.style.transform, 'translate(30px, 36px) scale(1)')
  reset.click()
  ...
  preview.dispatchEvent(new dom.window.MouseEvent('pointermove', { bubbles: true, clientX: 42, clientY: 56 }))
  preview.dispatchEvent(new dom.window.MouseEvent('pointerup', { bubbles: true }))
  assert.equal(canvas.style.transform, 'translate(30px, 36px) scale(0.6)')
  reset.click()
  assert.equal(canvas.style.transform, 'translate(0px, 0px) scale(1)')
  zoomIn.click()
...
test('Mermaid immersive view never invokes browser fullscreen, keeps its controls mutually exclusive, and cleans up on exit', async () => {
  ...
  Object.defineProperty(block, 'requestFullscreen', { configurable: true, value: () => { nativeFullscreenCalls += 1 } })
  ...
  assert.equal(dom.window.document.fullscreenElement ?? null, null)
  assert.equal(fullscreen.hidden, true)
  assert.equal(close.hidden, false)
  assert.equal(dom.window.document.body.classList.contains('mermaid-immersive-active'), true)
