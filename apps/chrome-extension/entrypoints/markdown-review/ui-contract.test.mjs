...
test('review actions use icon history controls and route rewrite or accept through the bound session', () => {
  ...
  assert.match(main, /adoptionBlockedReason/)
  assert.match(main, /title=\{acceptBlockedReason \?\? '在绑定会话中采纳并继续 Skill'\}/)
  assert.match(main, /disabled=\{sessionActionPending !== undefined \|\| state\.status === 'reopen-required' \|\| acceptBlockedReason !== undefined\}/)
