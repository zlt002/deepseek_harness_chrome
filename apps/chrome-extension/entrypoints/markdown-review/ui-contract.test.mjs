...
test('review actions use icon history controls and route rewrite or accept through the bound session', () => {
  ...
  assert.match(main, />↷<\/button>/)
  assert.match(main, />↻<\/button>/)
  assert.match(main, /↺ 删除重写/)
  assert.match(main, /✓ 写入在线文档/)
  ...
  assert.match(main, /revision: activeSnapshot\.resource\.revision, fingerprint: activeSnapshot\.resource\.fingerprint/)
  assert.doesNotMatch(main, /adoptionBlockedReason/)
  assert.match(main, /title=\{snapshot\?\.truncated === true \? '内容已截断，不能安全采纳' : '把执行指令放入右侧当前会话'\}/)
  assert.match(main, /disabled=\{sessionActionPending !== undefined \|\| state\.status === 'reopen-required' \|\| snapshot\?\.truncated === true\}/)
...
test('review tab offers five accessible stars with ten half-star choices linked to its generated PRD', () => {
  assert.match(main, /const PRD_STARS = \[1, 2, 3, 4, 5\] as const/)
  assert.match(main, /type: 'markdown-review-rating-request'/)
  assert.match(main, /aria-label="选择 0\.5 到 5 星评分，可选半星"/)
  assert.match(main, /这份 PRD 质量怎么样？/)
  assert.match(main, /花 3 秒点亮星星，你的反馈会直接帮助我们改进生成质量。/)
  assert.match(main, /function ratingDescription\(rating: PrdRating\)/)
  assert.match(main, /aria-describedby="prd-rating-help"/)
  assert.match(main, /const leftRating(?:: PrdRating)? = \(star === 1 \? 0\.5 : star - 0\.5\) as PrdRating/)
  assert.match(main, /const rightRating(?:: PrdRating)? = star as PrdRating/)
  assert.match(main, /aria-label=\{`\$\{leftRating\} 星，\$\{ratingDescription\(leftRating\)\}`\}/)
  assert.match(main, /aria-label=\{`\$\{rightRating\} 星，\$\{ratingDescription\(rightRating\)\}`\}/)
  assert.match(main, /aria-pressed=\{ratingPending === leftRating \|\| rating === leftRating\}/)
  assert.match(main, /aria-pressed=\{ratingPending === rightRating \|\| rating === rightRating\}/)
  assert.match(main, /onFocus=\{\(\) => setRatingPreview\(leftRating\)\}/)
  assert.match(main, /onFocus=\{\(\) => setRatingPreview\(rightRating\)\}/)
  assert.match(main, /ratingPreview !== undefined \? `\$\{ratingPreview\} 星 · \$\{ratingDescription\(ratingPreview\)\}`/)
  assert.match(main, /: '点击星星即可评分'/)
  assert.match(main, /snapshot\?\.pmdPrd === true && rating === undefined/)
  assert.match(main, /评分未提交：/)
  assert.match(main, /评分已提交，正在同步埋点。/)
  assert.match(main, /setRating\(message\.rating\)/)
  assert.match(style, /\.prd-rating\s*\{/)
  assert.match(style, /--review-rating-accent:/)
  assert.match(style, /background: linear-gradient\(115deg, color-mix\(in srgb, var\(--review-rating-soft\)/)
  assert.match(style, /box-shadow: 0 3px 12px/)
  assert.match(style, /\.prd-rating-star\s*\{/)
  assert.match(style, /\.prd-rating-star-fill\s*\{/)
  assert.match(style, /\.prd-rating-half\.is-left/)
  assert.match(style, /\.prd-rating-half\.is-right/)
  assert.match(style, /\.prd-rating-half:focus-visible\s*\{[\s\S]*outline: none;[\s\S]*box-shadow: none;/)
  assert.match(style, /\.prd-rating-star:has\(\.prd-rating-half:focus-visible\)/)
  assert.match(style, /\.prd-rating-label\s*\{[\s\S]*width: 170px;[\s\S]*flex: 0 0 170px;/)
  assert.doesNotMatch(style, /\.prd-rating-options button\.is-selected/)
})

test('PRD adoption asks to use the current side-panel session before it starts delivery', () => {
  assert.match(main, /const \[adoptionConfirmation, setAdoptionConfirmation\] = useState<MarkdownReviewSnapshot>\(\)/)
  assert.match(main, /setAdoptionConfirmation\(activeSnapshot\)/)
  assert.match(main, /if \(content === activeSnapshot\.content\) \{ runSessionAction\('accept', true\); return \}/)
  assert.match(main, /kind: 'prepare', content, adoption: true/)
  assert.match(main, /kind: 'commit', content: attempt\.active\.content, token: attempt\.active\.token, adoption: true/)
  assert.match(main, /if \(expected\.adoption\) \{[\s\S]*?setSessionActionPending\(undefined\)[\s\S]*?setAdoptionReady\(true\)/)
  assert.match(main, /保存已验证，但当前文件快照已失效。请重新读取后再试。/)
  assert.match(main, /adoptionConfirmation !== undefined && <div className="confirmation-dialog-backdrop">/)
  assert.match(main, /role="dialog" aria-modal="true" aria-labelledby="adoption-confirmation-title"/)
  assert.match(main, /id="adoption-confirmation-title">确认采纳/)
  assert.match(main, /确认把执行指令放到右侧当前会话吗？/)
  assert.match(main, />取消<\/button>/)
  assert.match(main, />确认<\/button>/)
  assert.match(main, /onClick=\{\(\) => setAdoptionConfirmation\(undefined\)\}/)
  assert.doesNotMatch(main, /chrome\.tabs\.update/)
  assert.doesNotMatch(main, /确认采纳[\s\S]{0,600}displayPath/)
})
