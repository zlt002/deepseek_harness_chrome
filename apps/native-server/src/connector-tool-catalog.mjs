...
const lightDocumentWritePreviewTool = { name: 'light_document_write_preview', title: 'Preview light-document write', description: 'Preview a non-selection light-document change on the current write target and return a one-time challenge. It does not change the document. For insert_drawing, use payload { mermaid, position: "end" }; Mermaid source is required and SVG is not accepted. After the user confirms, call light_document_write_commit with only that challenge.', annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: false, openWorldHint: false }, inputSchema: { type: 'object', additionalProperties: false, required: ['operation', 'payload'], properties: { operation: { enum: MODEL_LIGHT_DOCUMENT_OPERATIONS }, payload: { type: 'object', additionalProperties: true } }, allOf: [{ if: { properties: { operation: { const: 'insert_drawing' } }, required: ['operation'] }, then: { properties: { payload: { type: 'object', additionalProperties: false, required: ['mermaid'], properties: { mermaid: { type: 'string', minLength: 1, maxLength: 20000, description: 'Mermaid source, such as flowchart TD.' }, position: { enum: ['start', 'end', 'before', 'after'], description: 'Defaults to end; before/after also requires id or index.' }, id: { type: 'string', minLength: 1, maxLength: 256 }, index: { type: 'integer', minimum: 0, maximum: 100000 } } } } } }] } }
...
const teamKnowledgeBatchPreviewTool = { name: 'team_knowledge_batch_preview', title: 'Preview one to ten Team Knowledge light documents', description: 'The first step for one to ten light documents. Provide a stable batchId and the exact ordered names and bodies. A pmd: batch additionally requires the one-time pmdReviewReceipt issued by Markdown Review adoption; text that merely claims adoption is rejected. This inspects the currently bound parent and returns a model-visible one-time creation challenge plus expiresAt. Copy that exact challenge into team_knowledge_batch_create after explicit user confirmation. If it expired, preview the unchanged batch again before create. No online document is created in this step.', annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }, inputSchema: { type: 'object', additionalProperties: false, required: ['batchId', 'items'], properties: { batchId: { type: 'string', minLength: 1, maxLength: 128 }, items: { type: 'array', minItems: 1, maxItems: 10, items: teamKnowledgeBatchItemSchema }, pmdReviewReceipt: { type: 'string', minLength: 1, maxLength: 256, description: 'Required only for pmd: batches: the one-time receipt issued by the current Markdown Review adoption.' } } } }
