...
import { redactSensitiveDiagnostic } from '../telemetry/redact.mjs'
...
// Source modules live in src/runtime; bundled hosts keep their original base.
// Preserve installed resource, user workspace, and checkout resolution paths.
const THIS_DIR = basename(MODULE_DIR) === 'runtime' && basename(dirname(MODULE_DIR)) === 'src'
  ? dirname(MODULE_DIR)
  : MODULE_DIR
...
- insert:
    - id: deepseek-harness-selected-source-routing
      name: ${yamlString(loaderModuleSpecifier([resolve(THIS_DIR, 'knowledge/selected-source-routing-prompt.mjs'), resolve(THIS_DIR, 'selected-source-routing-prompt.mjs')].find(existsSync) ?? resolve(THIS_DIR, 'selected-source-routing-prompt.mjs')))}
