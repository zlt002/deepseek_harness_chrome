...
import { bundleHarnessDefaultWorkspacePlugin, bundleHarnessRuntimePlugin, bundleHarnessTrackingPlugin } from '../build/bundle-harness-runtime-plugin.mjs'
...
import { createRuntimeIdentity } from '../shared/runtime-identity.mjs'
import { materializeProductSkills } from '../skills/materialize-product-skills.mjs'
...
async function replaceDirectory(source, destination, prepare, copy = (from, to) => cp(from, to, { recursive: true, dereference: true })) {
...
await replaceDirectory(skillsSource, skills, undefined, materializeProductSkills)
