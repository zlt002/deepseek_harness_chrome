...
本 Skill 可在任意项目目录中使用；当前工作目录只用于绑定本 Run、放置本地过程状态和解析用户明确选择的本地项目，不限制业务类型，也不得要求切换到固定仓库或固定目录。远程代码库、知识库的授权和查询仍以本 Run 的独立 scope 为准，与当前工作目录分离。
...
在 Code Mode 中，每次顶层 `run_code` 调用都必须同时提供 `code` 和 `description`；程序内的工具调用也必须严格匹配当前 SDK schema。若收到参数校验错误，先根据错误补齐或纠正字段，再重试一次；不得使用相同参数重复调用。工具成功后直接进入当前阶段的用户输出，不为已完成的机械步骤追加长篇自我分析。
