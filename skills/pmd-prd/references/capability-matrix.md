...
| 5 单 PRD | 完整公司 PRD + 左侧 Markdown Review | 所有查询已结算/明确失败/明确跳过且影响分析 settled 后才可冻结；PRD 九个主章节顺序不变；标题和正文没有字段标签；每个功能包含现状、调整方式、输入/输出规则、调整后效果；第八章包含关联改动与风险、回归范围、异常关注点和五类验收；无编造事实或无依据数字；冻结并校验通过后调用 `open_workspace_markdown_review`，左侧“采纳”即直接进入远程同步；后台把采纳绑定当前 Run、会话、文件版本和正文 hash |
| 6 PRD 预览 | `mcp__chrome__team_knowledge_batch_preview` | 左侧“采纳”写入 `prd_sync_pending`；稳定 `batchId = pmd:${requirementId}` 已持久化在 Run state；目标已就绪则恰好一项已采纳 PRD 直接提交。目标未就绪时提示用户打开目标在线文档所在的目录标签、顶部选中后继续当前对话；下一次 continuation 自动同步，无需再次采纳。模型不传采纳凭据；Connector 内部核对同一会话、batchId、精确正文，Extension 回读同一文件版本。当前 Browser Target 与可创建父节点由 Connector 内部核对；目标或正文变化才中断 |
| 7 创建交付 | `mcp__chrome__team_knowledge_batch_create` | `create` 只传 `batchId`+`challenge` 且不重传正文；预览后立即创建，不再二次确认；PRD 有 catalogId/stages/同目标持久化 readback，batch 状态为 completed |
