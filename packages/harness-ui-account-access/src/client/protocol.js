...
export function createAccountAccessProtocol({ createStore, nonce, parentOrigin }) {
  ...
  return {
    ...
    probeGateway(apiKey, protocol, parent) {
      if (protocol !== 'anthropic-messages' && protocol !== 'openai-completions') throw new Error('Unsupported company gateway protocol.')
      gatewayOutgoing += 1
      const requestId = crypto.randomUUID()
      parent.postMessage({ type: 'company-gateway-probe-command/v1', nonce, sequence: gatewayOutgoing, requestId, apiKey, protocol }, parentOrigin)
      return requestId
    },
  }
}
