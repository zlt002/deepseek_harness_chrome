...
test('declares an out-of-tree account settings plugin without upstream source imports', async () => {
  ...
  assert.match(section, /requestedModelId = modelDrafts\[0\]\?\.id\.trim\(\)/)
  assert.match(section, /当前协议或首个模型尚未完成 Agent 工具验证/)
  ...
  assert.match(onboarding, /capability\.modelId !== models\[0\]\?\.id\.trim\(\)/)
  assert.match(onboarding, /requestedModelId = selectedModel\?\.trim\(\)/)
  assert.match(onboarding, /当前协议或所选模型尚未完成 Agent 工具验证/)
