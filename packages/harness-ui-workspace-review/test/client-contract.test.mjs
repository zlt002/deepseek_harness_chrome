...
test('renders through the picker child slot and desktop header action without a quick action', async () => {
  ...
  assert.match(client, /ctx\.sessions\.list\.getSnapshot\(\)\.current/)
  ...
  assert.match(client, /openState === 'open'/)
  assert.match(client, /tracker\.current\.activate\(sessionId, openState === 'open', review\)/)
  assert.doesNotMatch(client, /const autoOpenTracker = new WorkspaceMarkdownReviewOpenTracker\(\)/, 'opening state must not survive a hidden session unmount')
  ...
  assert.match(client, /if \(action\.action === 'rewrite'\)[\s\S]*existing\.trim\(\) === '' \? addition : `[\s\S]*?\$\{existing\}[\s\S]*?const currentSessionId = ctx\.sessions\.list\.getSnapshot\(\)\.current[\s\S]*?input\.setDraft\(acceptPrompt\(action\)\)/)
  assert.doesNotMatch(client, /if \(action\.action === 'accept'\)[\s\S]*ctx\.sessions\.open\(action\.harnessSessionId/)
  assert.doesNotMatch(client, /input\.submit/)
  assert.match(client, /acceptPrompt\(action\)/)
  assert.match(client, /action: 'accept',[\s\S]*status: 'draft_ready'/)
  assert.match(client, /return `\/pmd-prd 请把左侧已采纳的 PRD 写入当前打开的空白轻文档：\$\{action\.displayPath\}。如果当前不是空白轻文档，请直接告诉我。`/)
  assert.doesNotMatch(client, /light_document_read|Verified Write|同一目标回读|不得扫描或复用其他会话|历史交付/)
  assert.doesNotMatch(client, /team_knowledge_batch_preview|team_knowledge_batch_create|scopedConversation\.send/)
  assert.doesNotMatch(client, /accepted_revision|accepted_fingerprint|pmdReviewReceipt/)
