...
test('renders through the picker child slot and desktop header action without a quick action', async () => {
  ...
  assert.match(client, /currentWorkspaceReviewTarget/)
  assert.match(client, /sessions\.current/)
  assert.match(client, /sameWorkspaceCwd\(current\.cwd, document\.cwd\)/)
  assert.match(client, /submitWorkspaceMarkdown\(target\.targetSessionId, feedback\)/)
  assert.match(client, /status: current\.running \? 'queued' : 'processing'/)
