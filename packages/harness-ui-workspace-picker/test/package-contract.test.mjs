...
test('Claude Code import stays bounded, on-demand, cancellable, and uses public session verbs', async () => {
  ...
  assert.doesNotMatch(directory, /MAX_SOURCE_BYTES/, 'selected session files must not have a byte-size rejection gate')
  assert.match(directory, /MAX_SESSION_PAGE_SIZE = 64/)
  assert.match(directory, /readPrefix\(file, MAX_PREVIEW_BYTES\)/)
  assert.match(directory, /createReadStream\(file, \{ encoding: 'utf8', signal \}\)/, 'selected details and imports must stream rather than retain the source file')
  assert.match(directory, /async detail\(/, 'details must remain an on-demand Host operation')
  assert.match(host, /action === 'detail'/)
  assert.match(host, /action === 'import'/)
  assert.match(host, /importNativeHistory/)
  assert.match(host, /'agentPresets'/, 'default preset service must be explicitly injected')
  assert.doesNotMatch(host, /session\.prompt/)
  ...
  assert.doesNotMatch(client, /session\.prompt/)
  assert.match(client, /openImportedSession\(ctx\.sessions/)
  const opener = await read('src/client/open-imported-session.mjs')
  assert.match(opener, /await sessions\.refresh\(\)/)
  assert.match(opener, /sessions\.open\(sessionId\)/)
  ...
  assert.match(modal, /正在写入原生历史，不会自动请求模型/)
  assert.match(modal, /重新导入为副本/)
  assert.match(modal, /查看详情/)
  assert.match(modal, /action: 'detail'/)
  assert.match(modal, /aria-label=\{`查看 \$\{session\.title\} 详情`\}/)
  assert.match(modal, /role="dialog" aria-modal="true" aria-labelledby=\{detailTitleId\}/)
  assert.match(modal, /正在按需读取会话详情/)
  assert.match(modal, /仅显示最近 120,000 个字符/)
  ...
  assert.match(css, /\.importSessionRow:hover \.importDetailButton, \.importSessionRow:focus-within \.importDetailButton/, 'the details entry must work on hover and keyboard focus without replacing the date')
  assert.match(css, /\.importDetailNotice\s*\{/, 'truncated detail content must be visibly disclosed')
