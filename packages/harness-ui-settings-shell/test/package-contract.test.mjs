...
test('Settings shell is product-owned and composes official settings slots', async () => {
  ...
  assert.match(index, /settingsScope\.bind\(\{ namespace: 'ui-theme' \}\)/)
  assert.match(index, /hasSavedPreference/)
  assert.match(index, /setTheme\('light'\)/)
  assert.match(index, /themeSettings\.subscribe\(applyDefaultTheme\)/)
  assert.match(index, /if \(!\/Windows\/i\.test\(navigator\.userAgent\)\) return/)
  ...
  assert.match(root, /IconUserOutline16/)
  assert.match(root, /id === 'accrui-account'/)
  assert.match(root, /IconDownloadOutline16/)
  assert.match(root, /id === 'accrui-release-update'/)
