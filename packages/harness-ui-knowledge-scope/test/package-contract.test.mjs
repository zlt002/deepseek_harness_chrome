...
test('scope pickers use the shared upward overlay and preserve the accepted e327 geometry', async () => {
  ...
  assert.match(control, /<IconCodeBrackets16 \/>/)
  assert.match(control, /function IconCodeBrackets16\(\)[\s\S]*stroke="currentColor"/)
  assert.match(control, /M9\.5 3L6\.5 13/)
  assert.match(control, /<IconBookOutline16 \/>/)
  assert.match(control, /function IconBookOutline16\(\)[\s\S]*stroke="currentColor"/)
