...
export function KnowledgeScopeStrip({ session, useSession, useKnowledgeScope, request }: StripProps) {
  ...
  return <><div className={css.strip} aria-label="知识检索范围">
    ...
    <button className={css.repositoryToggle} type="button" role="switch" aria-label="启用知识查询" aria-checked={enabled} onFocus={() => setRememberOpen(true)} onClick={() => updateSwitch({ enabled: !enabled }, scope ?? { domainSystems: {}, repositoryIds: [] })}>
...
function KnowledgeScopePanelBody({ sessionId, useKnowledgeScope, request, section }: PanelProps & { section: 'knowledge' | 'repositories' }) {
  ...
  const catalog = snapshot?.catalog ?? emptyCatalog
  const scope = scopeFor(snapshot, id) ?? { domainSystems: {}, repositoryIds: [] }
  ...
  return <div ref={panelRef} className={css.panel} role="dialog" aria-label={section === 'knowledge' ? '知识范围' : '代码库范围'} style={maxHeight === undefined ? undefined : { maxHeight }}>
    ...
    {section === 'knowledge' ? <div className={css.section} aria-label="知识库范围">
      ...
      <div className={`${css.tree} ${css.knowledgeTree}`}>{catalog.domains.map(domain => {
        ...
        const selected = domain.id in draftScope.domainSystems
        const systems = catalog.systems.filter(system => system.domainId === domain.id)
        const systemIds = systems.map(system => system.id)
        const selectedCount = (draftScope.domainSystems[domain.id] ?? []).filter(systemId => systemIds.includes(systemId)).length
        ...
        return <div key={domain.id} className={css.systemGroup}>
          ...
          {expanded && <div className={css.repositoryList}>{systems.map(system => <label key={system.id} className={css.option}><input aria-label={system.name} type="checkbox" checked={(draftScope.domainSystems[domain.id] ?? []).includes(system.id)} onChange={(event) => update(selectKnowledgeSystem(draftScope, domain.id, system.id, event.target.checked, systemIds))}/><span>{system.name}</span></label>)}</div>}
        </div>
      })}</div>
