...
function BrowserTargetPanelBody({ useBrowserTarget, onBrowserTargetCommand }: PanelProps) {
  ...
  return <div ref={panelRef} className={css.panel} role="dialog" aria-label="工作目标上下文">
    ...
    <section className={`${css.referenceGuide} ${captureBusy ? css.referenceGuideBusy : ''}`} aria-label="制作 AI 原型" role={captureBusy ? 'status' : undefined}><strong>{captureBusy ? `正在提取：${displayName(capturingTab)}` : '制作 AI 原型'}</strong><span>{captureBusy ? '正在读取完整样式并校验截图，通常几秒；大页面最多等待 45 秒。请勿重复点击或重新加载，完成后会自动打开确认页。' : '点击任意网页右侧的“提取设计规范”。无需先勾选，也无需设为主目标。'}</span></section>
