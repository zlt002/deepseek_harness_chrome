...
function BrowserTargetPanelBody({ useBrowserTarget, onBrowserTargetCommand }: PanelProps) {
  ...
  return <div ref={panelRef} className={css.panel} role="dialog" aria-label="工作目标上下文">
    ...
    <p className={css.note}>{pinned ? '固定的是勾选的浏览器标签，不是当时的网址。同一标签换了文档时，下一次读取会跟最新内容；关掉标签才失效。主目标用于默认页面和未来写操作。' : snapshot.settings.mode === 'none' ? '下一次浏览器工具调用会明确提示已关闭浏览器能力。' : '发送后，本次运行会固定发送瞬间的 Browser Target；运行结束后恢复跟随。'}</p>
