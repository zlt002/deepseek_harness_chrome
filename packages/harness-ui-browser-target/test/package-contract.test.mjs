...
test('awaits the follow-mode lock acknowledgement, preserves accepted locks across surface disposal, then reconciles every idle session', async () => {
  ...
  assert.match(client, /const browserTarget = targetSnapshot\?\.settings\.mode === 'follow-active-tab'[\s\S]*?targetSnapshot\.activeTab[\s\S]*?targetSnapshot\?\.settings\.mode === 'pinned-tabs' \? browserTargetTriggerTab\(targetSnapshot\) : undefined/, 'follow captures the active target; pinned captures its selected primary; none captures nothing')
  ...
  assert.match(client, /const lifecycle = \{ state: new BrowserTargetSessionRunLock\(submissionId\)/)
  assert.match(runLock, /accepted = false/)
  assert.match(runLock, /observedActivity = false/)
  assert.match(runLock, /restored = false/)
  assert.match(runLock, /snapshot\.running \|\| snapshot\.queue\.length > 0/)
  assert.match(runLock, /snapshot\.blank === true/)
  assert.match(runLock, /this\.accepted && idle && \(this\.observedActivity \|\| restoredPromptNeverAccepted\)/)
  assert.match(runLock, /shouldReconcileSessionRunTarget/)
  assert.match(runLock, /const completed = lock\?\.observe\(snapshot\)/, 'reconciliation records queue activity even when the current snapshot is not idle')
  assert.match(client, /const lifecycle = lifecycleLocks\.get\(sessionId\)/)
  assert.match(client, /shouldReconcileSessionRunTarget\(snapshot, lifecycle\.state\)/)
  assert.match(client, /lifecycleLocks\.set\(id, lifecycle\).*await lockSubmission/s, 'the idle sweep sees a pending submission lock before its acknowledgement')
  ...
  assert.match(client, /if \(expected\.has\(sessionId\)\) continue[\s\S]*?releaseLifecycleLock\(sessionId\)/, 'removing a session must release its accepted Browser Target lock')
  assert.match(client, /type: 'browser-target-reconcile\/v1', nonce: config\.nonce, sessionId, submissionId/)
  assert.match(client, /activeRunLock/)
  assert.match(client, /activeRunLocks/)
  assert.match(client, /for \(const projected of projectedLocks\)/)
  assert.match(client, /state\.accept\(session\.getSnapshot\(\)\)/)
  assert.match(client, /postReconcile\(sessionId, lifecycle\.state\.submissionId\)/)
