...
type OfficeSpreadsheetPrecondition = OfficeSpreadsheetPreconditionV1 | OfficeSpreadsheetPreconditionV2 | OfficeSpreadsheetPreconditionV3 | OfficeSpreadsheetPreconditionV4
...
function isOfficeSpreadsheetRequest(message: NativeMessage): message is OfficeSpreadsheetRequest {
  ...
  if (!(message.type === 'connector_request' && typeof message.requestId === 'string' && typeof message.runId === 'string' && typeof message.generation === 'string'
    ...
    && ['context', 'range', 'range_features', 'search', 'sheets', 'defined_names', 'capabilities', 'view', 'inspect_write', 'write'].includes(String(candidate.action)))) return false
