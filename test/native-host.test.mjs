...
test('records a PMD review adoption only through the active Connector Run', async () => {
  const recorded = []
  const messages = []
  ...
  assert.deepEqual(recorded, [{ runId: 'run-adoption', ...payload }])
  assert.deepEqual(messages.at(-1), { type: 'pmd_prd_review_adoption_recorded', requestId: 'request-adoption-1' })
