...
test('buildWindowsRelease creates the AccrUI updater contract with the fixed extension identity', async () => {
  ...
  assert.match(installLauncher, /If nonInteractive And exitCode <> 0 Then/)
...
test('the in-place updater start script re-registers both native-host names through one Node-gated script', async () => {
  ...
  assert.match(installer, /Stop-InstalledProductProcesses \$installRoot[\s\S]*Move-ManagedTree \$installRoot \$previousRoot/)
...
test('Windows Native Messaging acceptance preserves cmd launcher quoting and fails closed on smoke errors', async () => {
  ...
  assert.match(acceptanceSource, /function Start-InstalledRuntimeLockHolder/)
  assert.match(acceptanceSource, /Old installed runtime lock holder was stopped before upgrade/)
