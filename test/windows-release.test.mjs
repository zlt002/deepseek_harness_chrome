...
test('the in-place updater start script re-registers both native-host names through one Node-gated script', async () => {
  ...
  assert.ok(extensionInstall.indexOf("Copy-ExtensionFileAtomically $manifestFile (Join-Path $Destination 'manifest.json')") < extensionInstall.indexOf('foreach ($file in @(Get-ChildItem -LiteralPath $Destination -Recurse -File))'), 'manifest.json must commit before stale-file removal')
