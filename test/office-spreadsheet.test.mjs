...
test('validates dimension write scope and rejects forged per-item hidden or size readback', async () => {
  ...
  const connector = new BrowserConnector({ officeSpreadsheetWriteStore: writeStore(), requestExtension: (request) => queueMicrotask(() => { const result = request.action === 'write' ? { status: 'verified_write', resource, operation: request.operation, requested: { range: '1:2', axis: 'row', hidden: true }, observed: { dimensions: { ...dimensions, items: forge ? [{ index: 1, hidden: true, size: 15 }, { index: 2, hidden: true, size: 16 }] : dimensions.items.map((item) => ({ ...item, hidden: true })) }, verified: true } } : { status: 'ok', resource, precondition }; connector.acceptExtensionResponse({ type: 'connector_response', requestId: request.requestId, runId: request.runId, generation: request.generation, browserTarget: target, result }) }) })
  connector.bindBrowserTarget('spreadsheet-dimension-run', target); const endpoint = await connector.start()
  try {
    const invalid = await call(endpoint, { action: 'inspect_write', operation: 'set_rows_hidden', payload: { range: 'A:B', hidden: true } }); assert.equal(invalid.error.code, -32602)
    const inspected = await call(endpoint, { action: 'inspect_write', operation: 'set_rows_hidden', payload: { range: '1:2', hidden: true } }); const success = await call(endpoint, { action: 'write', challenge: inspected.result.structuredContent.challenge, idempotencyIdentity: 'dimension-success', resource, operation: 'set_rows_hidden', payload: { range: '1:2', hidden: true } }); assert.equal(success.result.structuredContent.status, 'verified_write')
    forge = true; const second = await call(endpoint, { action: 'inspect_write', operation: 'set_rows_hidden', payload: { range: '1:2', hidden: true } }); const rejected = await call(endpoint, { action: 'write', challenge: second.result.structuredContent.challenge, idempotencyIdentity: 'dimension-forged', resource, operation: 'set_rows_hidden', payload: { range: '1:2', hidden: true } }); assert.equal(rejected.result.isError, true); assert.match(rejected.result.content[0].text, /invalid verified spreadsheet write/i)
  } finally { await connector.stop() }
})

test('batch_write requires a complete rectangle and rejects a forged non-value state readback', async () => {
  const target = { browser: 'chrome', windowId: 4, tabId: 41, url: 'https://doc.midea.com/sheets/batch' }; const resource = { kind: 'webedit_spreadsheet', origin: 'https://webedit.midea.com', workbookName: 'Batch.xlsx', sheetName: 'Sheet1', fingerprint: 'batch-sheet' }
  const baseline = writePrecondition('A1:B2'); const precondition = { ...baseline, state: { ...baseline.state, formulas: [['', ''], ['', '']] } }; const payload = { cells: [{ cell: 'A1', value: 1 }, { cell: 'B1', value: 2 }, { cell: 'A2', value: 3 }, { cell: 'B2', value: 4 }] }; let forge = false
  const connector = new BrowserConnector({ officeSpreadsheetWriteStore: writeStore(), requestExtension: (request) => queueMicrotask(() => { const state = { ...precondition.state, values: [[1, 2], [3, 4]], ...(forge ? { format: { ...precondition.state.format, bold: true } } : {}) }; const result = request.action === 'write' ? { status: 'verified_write', resource, operation: 'batch_write', requested: { range: 'A1:B2', cells: payload.cells }, observed: { range: 'A1:B2', values: [[1, 2], [3, 4]], formulas: [['', ''], ['', '']], state, verified: true } } : { status: 'ok', resource, precondition }; connector.acceptExtensionResponse({ type: 'connector_response', requestId: request.requestId, runId: request.runId, generation: request.generation, browserTarget: target, result }) }) })
  connector.bindBrowserTarget('spreadsheet-batch-run', target); const endpoint = await connector.start()
  try {
    const invalid = await call(endpoint, { action: 'inspect_write', operation: 'batch_write', payload: { cells: [{ cell: 'A1', value: 1 }, { cell: 'B2', value: 2 }] } }); assert.equal(invalid.error.code, -32602)
    const inspected = await call(endpoint, { action: 'inspect_write', operation: 'batch_write', payload }); const success = await call(endpoint, { action: 'write', challenge: inspected.result.structuredContent.challenge, idempotencyIdentity: 'batch-success', resource, operation: 'batch_write', payload }); assert.equal(success.result.structuredContent.status, 'verified_write')
