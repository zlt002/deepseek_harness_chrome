...
test('mounts the Harness-native pmd-prd skill with its template contract', async () => {
  ...
  assert.match(skill, /每个子项对应的 PC 页面 URL、前端代码文件、后端代码文件\/接口/)
  ...
  for (const heading of ['4.1 改动点', '4.1.1 按钮', '定位项', '原逻辑', '调整后逻辑', '交互与规则', '验收清单']) assert.match(template, new RegExp(heading))
