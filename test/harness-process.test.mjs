...
test('mounts the Harness-native pmd-prd skill with its template contract', async () => {
  ...
  for (const tool of ['light_document_read', 'light_document_write_preview', 'light_document_write_commit']) assert.match(skill, new RegExp(tool))
  assert.doesNotMatch(skill, /mcp__chrome__team_knowledge_batch_preview|mcp__chrome__team_knowledge_batch_create/)
  ...
  assert.match(skill, /prd_review → target_selection → verified_write/)
  ...
  assert.match(skill, /适用页面（仅适用时）、与每条目标修改点直接相关的前端\/后端完整相对代码路径、接口、稳定位置/)
  ...
  assert.match(skill, /pmd-workspace\/spec\/<requirementId>\/<requirementId>_\*_PRD\.md/)
  ...
  assert.doesNotMatch(template, /### 功能：|改动总览与影响\n\n## 页面与代码定位总览/)
...
test('declares pmd-prd run binding, isolated scope, persisted state, and stale-confirmation recovery contracts', async () => {
  ...
  assert.match(processState, /用户勾选后收到任意继续消息，或 Ask 返回“已选好资料，立即重新读取范围并继续”时，都必须在同一次 continuation 立即重新调用一次 `mcp__chrome__selected_source_scope`/)
  ...
  assert.match(processState, /阶段 5 确认正文来自左侧 Markdown Review 当前编辑器/)
  assert.match(processState, /阶段 6 由用户在当前 Browser Target 选择或新建空白轻文档/)
  assert.match(processState, /阶段 7 的用户手动发送执行指令是唯一写入确认/)
  assert.match(processState, /light_document_read → preview → commit → readback/)
  ...
  assert.match(skill, /最后一次采纳只保留这一条待执行指令/)
  ...
  assert.match(skill, /同一目标返回 `verified_write`/)
  assert.doesNotMatch(skill, /batchId|team_knowledge_batch_create|team_knowledge_batch_preview/)
...
test('keeps pmd-prd project-agnostic and bounds Code Mode execution', async () => {
  ...
  assert.match(skill, /用户勾选后收到任意继续消息，或 Ask 返回“已选好资料，立即重新读取范围并继续”时，都在同一次 continuation 立即重新调用一次 `mcp__chrome__selected_source_scope`/)
  ...
  assert.match(skill, /首次代码或知识检索的 `prompt` 只使用当前用户消息里的原始业务需求/)
  assert.match(skill, /`\/pmd-prd  优化下客户管理功能` 传 `优化下客户管理功能`/)
  assert.match(skill, /通用分析清单在检索结果回来后用于父会话核对证据和识别缺口，不加入首次 `prompt`/)
  assert.match(skill, /只有上一轮回执已经给出文件、接口或引用等真实证据/)
  assert.match(skill, /证据不足时继续使用原始业务需求/)
  assert.doesNotMatch(skill, /阶段 2 的代码检索 prompt 还必须逐个要求返回/)
...
test('keeps pmd-prd user output decision-first while hiding internal process details', async () => {
  ...
  assert.match(skill, /pmd_prd_reference_sources/)
  assert.match(skill, /选择资料（推荐）/)
  assert.match(skill, /自动打开真实代码库选择面板/)
  assert.match(skill, /已选好资料，立即重新读取范围并继续/)
  assert.match(skill, /同一次 continuation.*立即回读 `mcp__chrome__selected_source_scope`/)
  assert.match(processState, /同一次 continuation.*立即重新调用一次 `mcp__chrome__selected_source_scope`/)
  assert.match(skill, /跳过远程研究并直接进入阶段 3/)
...
test('keeps selected-source routing in the final Code preset system prompt', async () => {
  ...
  assert.match(prompt, /General analysis checklists apply only after real search evidence returns/)
  ...
  assert.match(prompt, /For a direct search, pass the current end user's message unchanged as prompt/)
