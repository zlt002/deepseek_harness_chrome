...
test('offers reference capture on a Browser Target and keeps the command nonce-bound', async () => {
  ...
  assert.doesNotMatch(control, /当前窗口标签页/)
  assert.doesNotMatch(control, /制作 AI 原型/)
  assert.doesNotMatch(control, /无需先勾选，也无需设为主目标/)
  assert.match(control, /合并提取设计规范/)
  assert.match(control, /合并参考/)
  assert.match(control, /className=\{css\.listActions\}/)
