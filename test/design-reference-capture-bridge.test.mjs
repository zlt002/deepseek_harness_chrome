...
test('offers reference capture on a Browser Target and keeps the command nonce-bound', async () => {
  ...
  assert.match(control, /大页面最多等待 45 秒/)
  assert.match(control, /请勿重复点击或重新加载/)
  assert.match(control, /role=\{captureBusy \? 'status' : undefined\}/)
  assert.match(control, /将切换到该网页[\s\S]*打开原型页/)
...
test('background capture validates the Side Panel and the same Browser Target before persisting', async () => {
  ...
  assert.match(background, /截图前参考网页被切换/)
  assert.match(background, /截图时参考网页被切换/)
  assert.match(background, /activeTabChangedDuringScreenshot/)
  assert.match(background, /chrome\.tabs\.onActivated\.addListener\(recordScreenshotTabChange\)/)
  assert.match(background, /chrome\.tabs\.onActivated\.removeListener\(recordScreenshotTabChange\)/)
