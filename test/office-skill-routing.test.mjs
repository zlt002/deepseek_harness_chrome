...
test('light-document skill separates blank, bounded-block, and whole-document writes by the supported contract', async () => {
  ...
  assert.match(source, /insert_drawing[\s\S]*position: "after_selection"[\s\S]*expectedSelectionFingerprint/)
  assert.match(source, /xychart-beta[\s\S]*未经当前 WebEdit 验证/)
  assert.match(source, /blocks_delete[\s\S]*blocks: \[\{ id \}\][\s\S]*不接受.*index/)
  ...
  assert.match(source, /删除完整稳定块用 `blocks_delete` 的公开 payload `\{ blocks: \[\{ id \}\] \}`；只接受本次读取的 id，不接受 index。只改标题用 `set_title`/)
