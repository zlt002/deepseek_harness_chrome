...
test('accepts the published Windows Lite common-root archive only when identity and version advance', () => {
  ...
  const verified = verifyWindowsLitePackage(archive, { currentVersion: '1.1.75' })
  assert.equal(verified.version, '1.1.76')
  assert.equal(verified.extensionId, 'cmgjacoohdgjedoekbdbhbelpmboankg')
  assert.throws(() => verifyWindowsLitePackage(archive, { currentVersion: '1.1.76' }), /未高于当前版本/)
  ...
  assert.throws(() => verifyWindowsLitePackage(missingKeyArchive), /固定身份 key/)
})
