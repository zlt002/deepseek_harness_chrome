...
import { PRESENTATION_WRITE_OPERATIONS } from '../apps/native-server/src/transport/connector-tool-catalog.mjs'

test('delivers Office requests and every page-world runtime to discovered WebEdit iframes', async () => {
  const [background, content, config, runtime, presentationRuntime, frameRouting] = await Promise.all([
    ...
    readFile(new URL('../apps/chrome-extension/entrypoints/background/office/frame-routing.ts', import.meta.url), 'utf8'),
  ])
  ...
  assert.match(frameRouting, /const OFFICE_CONTENT_SCRIPT_FILES = \['content-scripts\/office-read\.js'\]/)
