...
test('Windows CI builds from the recursive upstream submodule and materialized product tree', async () => {
  ...
  const buildExtension = workflow.indexOf('run: pnpm build', materializeRuntime)
  ...
  assert.ok(install < buildProduct && buildProduct < materializeRuntime && materializeRuntime < buildExtension && buildExtension < buildRelease)
