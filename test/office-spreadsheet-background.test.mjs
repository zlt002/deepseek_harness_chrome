...
test('routes office_spreadsheet writes only to the WebEdit iframe and forwards runtime failures', async () => {
  ...
  try {
    ...
    listeners.forEach((listener) => listener({ type: 'connector_request', requestId: 'sheet-activate', runId: 'spreadsheet-background-run', generation: 'g-1', browserTarget: target, tool: 'office_spreadsheet', action: 'inspect_write', operation: 'activate_worksheet', payload: { sheetName: 'Summary' } }))
    listeners.forEach((listener) => listener({ type: 'connector_request', requestId: 'sheet-copy-rejected', runId: 'spreadsheet-background-run', generation: 'g-1', browserTarget: target, tool: 'office_spreadsheet', action: 'inspect_write', operation: 'copy_worksheet', payload: { sourceName: 'Summary', newName: 'Copy' } }))
    await new Promise((resolve) => setTimeout(resolve, 0))
    assert.deepEqual(sent.at(-1), { tabId: 42, message: { type: 'office-spreadsheet/v1', action: 'inspect_write', operation: 'activate_worksheet', payload: { sheetName: 'Summary' } }, options: { frameId: 17 } })
    ...
    listeners.forEach((listener) => listener({ type: 'connector_request', requestId: 'sheet-print-invalid', runId: 'spreadsheet-background-run', generation: 'g-1', browserTarget: target, tool: 'office_spreadsheet', action: 'inspect_write', operation: 'set_print_settings', payload: { zoom: 100, fitToPagesWide: 1 } }))
    listeners.forEach((listener) => listener({ type: 'connector_request', requestId: 'sheet-outline-invalid', runId: 'spreadsheet-background-run', generation: 'g-1', browserTarget: target, tool: 'office_spreadsheet', action: 'inspect_write', operation: 'set_outline_group', payload: { range: 'A1:B2', axis: 'row', grouped: true } }))
    listeners.forEach((listener) => listener({ type: 'connector_request', requestId: 'sheet-outline-safe', runId: 'spreadsheet-background-run', generation: 'g-1', browserTarget: target, tool: 'office_spreadsheet', action: 'outline', range: '1:2', axis: 'row' }))
    listeners.forEach((listener) => listener({ type: 'connector_request', requestId: 'sheet-dimension-invalid', runId: 'spreadsheet-background-run', generation: 'g-1', browserTarget: target, tool: 'office_spreadsheet', action: 'inspect_write', operation: 'set_rows_hidden', payload: { range: 'A:B', hidden: true } }))
    listeners.forEach((listener) => listener({ type: 'connector_request', requestId: 'sheet-batch-invalid', runId: 'spreadsheet-background-run', generation: 'g-1', browserTarget: target, tool: 'office_spreadsheet', action: 'inspect_write', operation: 'batch_write', payload: { cells: [{ cell: 'A1', value: 1 }, { cell: 'A1', value: 2 }] } }))
    listeners.forEach((listener) => listener({ type: 'connector_request', requestId: 'sheet-batch-sparse', runId: 'spreadsheet-background-run', generation: 'g-1', browserTarget: target, tool: 'office_spreadsheet', action: 'inspect_write', operation: 'batch_write', payload: { cells: [{ cell: 'A1', value: 1 }, { cell: 'B2', value: 2 }] } }))
    listeners.forEach((listener) => listener({ type: 'connector_request', requestId: 'sheet-batch-out-of-bounds', runId: 'spreadsheet-background-run', generation: 'g-1', browserTarget: target, tool: 'office_spreadsheet', action: 'inspect_write', operation: 'batch_write', payload: { cells: [{ cell: 'XFE1', value: 1 }] } }))
    listeners.forEach((listener) => listener({ type: 'connector_request', requestId: 'sheet-dimension-safe', runId: 'spreadsheet-background-run', generation: 'g-1', browserTarget: target, tool: 'office_spreadsheet', action: 'dimensions', range: 'A:B', axis: 'column' }))
    listeners.forEach((listener) => listener({ type: 'connector_request', requestId: 'sheet-special-invalid', runId: 'spreadsheet-background-run', generation: 'g-1', browserTarget: target, tool: 'office_spreadsheet', action: 'special_cells', range: 'A1:XFD1048576', kind: 'constants' }))
    listeners.forEach((listener) => listener({ type: 'connector_request', requestId: 'sheet-special-safe', runId: 'spreadsheet-background-run', generation: 'g-1', browserTarget: target, tool: 'office_spreadsheet', action: 'special_cells', range: 'A1:B2', kind: 'constants', offset: 0, limit: 1 }))
    await new Promise((resolve) => setTimeout(resolve, 0))
    assert.equal(sent.length, 9)
    assert.deepEqual(sent.at(-1), { tabId: 42, message: { type: 'office-spreadsheet/v1', action: 'special_cells', range: 'A1:B2', kind: 'constants', offset: 0, limit: 1 }, options: { frameId: 17 } })
