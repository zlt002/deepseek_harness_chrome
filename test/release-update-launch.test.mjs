...
test('detached updater writes a standalone script and waits for ready before handing cleanup to the installer', async () => {
  ...
  try {
    ...
    assert.deepEqual(invocation[1].slice(0, -1), ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-WindowStyle', 'Hidden', '-File'])
    const updaterScriptPath = invocation[1].at(-1)
    ...
    assert.match(script, /& powershell\.exe -NoProfile -ExecutionPolicy Bypass -File '[^']*install\.ps1' -InstallRoot \$installRoot/)
...
test('detached updater falls back to a generic error when it exits before ready without a persisted cause', async () => {
  ...
  try {
    ...
    child.emit('spawn')
    child.emit('exit', 1)
    await assert.rejects(launched, /就绪握手前退出/)
