...
test('HTML Workbench keeps file permission, DOM selection, approval, and same-target readback fail-closed', async () => {
  ...
  assert.match(connector, /expectedStylesheets/)
  assert.match(background, /stylesheetFingerprints/)
  assert.match(background, /computedStyle/)
  assert.match(background, /bypassCache: true/)
