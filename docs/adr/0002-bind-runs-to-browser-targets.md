...
Saving this policy never interrupts an in-flight Connector request and does not require reconnecting. Pinned mode follows the checked tab (`tabId`), not the URL saved when the tab was checked. Same-tab navigation or a window move stays available and is transferred on the next Connector turn with the live window and URL; only a closed tab is reported unavailable. Pinned mode exposes every still-valid checked tab in `list_work_tabs`, marks the selected primary tab, and reports checked tabs that closed without blocking the remaining tabs. Legacy singular `browserTarget` and `officeContext` fields continue to represent that primary tab. The primary remains the default and the only future write target; the model cannot nominate a tab. None mode rejects the next browser tool call. The model does not open tabs. The user opens pages and checks them; the Connector only reads the bound work tabs.
