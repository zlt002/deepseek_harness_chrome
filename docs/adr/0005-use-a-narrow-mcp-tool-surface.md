...
The first milestone connects Harness to a Native Host-managed local MCP endpoint and exposes only six Connector Tools: `office_get_context`, `office_read_range`, `office_write_range`, `knowledge_search`, `code_search`, and `team_doc_create`. Short operations remain synchronous; long knowledge or delivery work becomes cancellable Connector Jobs. Generic Harness tool cards are sufficient, and no deep Cordis UI plugin is required until a proven product need emerges.
