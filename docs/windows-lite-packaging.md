...
版本。目前默认值是 `1.1.89`。发布新版本时应手动传入 `version`，或者同步修改
...
```sh
gh release view windows-lite-v1.1.89-candidate \
  --json targetCommitish,assets
gh release download windows-lite-v1.1.89-candidate \
...
```sh
gh release view windows-lite-v1.1.89 \
