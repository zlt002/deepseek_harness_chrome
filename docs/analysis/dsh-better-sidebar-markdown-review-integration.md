...
| P1 | 文件行支持“引用到对话”、复制相对路径、打开新 Tab/侧边分屏（[FileTree.tsx L101-L147](https://github.com/omdsh-dev/DSH-better-sidebar/blob/6c891514b544b6e2da51fdab2eb3436cc02da246/src/client/FileTree.tsx#L101-L147)，[L256-L280](https://github.com/omdsh-dev/DSH-better-sidebar/blob/6c891514b544b6e2da51fdab2eb3436cc02da246/src/client/FileTree.tsx#L256-L280)）。 | 当前点击 Markdown 会复用独立 Review Tab；批注发送到点击时侧边栏选中的同工作区会话，会话忙碌时走 Harness 原生队列。 | 不再增加第二种“直接塞路径到 composer”通道；可借鉴复制相对路径与刷新，继续保持单一 Review Tab 打开语义。 |
...
结论：下载源码**有用**，最值得吸收的是“有预算、可取消的文件名搜索”“显式刷新”“路径复制”和局部错误/加载状态；现有 capability、精确锚点、同工作区当前会话投递与 Browser Target 隔离应保持，因为它们比外部实现更适合 Chrome sidepanel 的安全边界。源码可按 MIT 参考，但当前没有必要复制其组件或把它作为 runtime 依赖。
...
4. 用户框选文本、填写批注，审阅 Tab 生成批注项（引用、锚点、创建时指纹），并点“送入会话”。目标是点击时 Side Panel 当前选中的同工作区会话；空闲时立即处理，运行中时走 Harness 原生队列。Review Tab 必须显示目标会话和“正在发送 / 已排队 / AI 处理中 / 候选已返回 / 失败”状态。Side Panel 未打开或未就绪时不得静默发送，应提示用户打开后重试；跨工作区会话必须拒绝。
